...
const float SPEED = .156;

const float STRIP_CHARS_MIN =  7.;
const float STRIP_CHARS_MAX = 40.;
const float STRIP_CHAR_HEIGHT = 0.04;
const float STRIP_CHAR_WIDTH = 0.028;
const float ZCELL_SIZE = 1. * (STRIP_CHAR_HEIGHT * STRIP_CHARS_MAX);  //the multiplier can't be less than 1.
const float XYCELL_SIZE = 36. * STRIP_CHAR_WIDTH;  //the multiplier can't be less than 1.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 blendedColor = mix(terminalColor.rgb * 1.2, col * 0.6, mask);
